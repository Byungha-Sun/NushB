'''
#1. 이름이 printName이고 파라메터로 문자열 하나를 받아서 간단히 받은 파라메터값을 출력하는 함수를 정의하고 호출(리턴값 없음)
def printName(str):
    print(str)
...
#3. 파라메터로 리스트 하나를 받아서 리스트에 담긴 숫자 모두의 합을 구하여 반환하는 함수를 정의 및 호출. 함수 이름은 sumNum
def sumNum(list):
    res=sum(list)
    print(res)
...
#4. 파라메터로 문자열하나 숫자하나를 받는 이름이 f1인 함수 정의. 호출시 파라메터의 순사가 바뀌어도 동작되도록 정의 및 호출하시오. 동작은 파라메터 값 출력하고 리턴값은 없음
...
#파라메터의 기본값이 지정된것과 지정되지 않은것을 섞어서 쓸 때 순서가 중요. 기본값 없는 파라메터가 먼저 정의되어야함
def f1(c,a=1,b=2):
    print(c,a,b)
