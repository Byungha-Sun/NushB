import pymysql
import healthCenter.MemberModel as mem
...
class LockerDao:
    ...
    def selectAll(self):
        ...
        try:
            cur.execute(sql)
            for row in cur:
                lockers.append(LockerVo(l_id=row[0], l_status=row[1]))
            return lockers
        except Exception as e:
            print(e)
        finally:
    ...
    def l_id_from_member(self,mem_user_id):
        self.connect()
        cur = self.conn.cursor()
        sql = "select l_id from member where mem_user_id=%s"
        vals = (mem_user_id)
        try:
            cur.execute(sql, vals)
            row = cur.fetchone()
...
class LockerService:
    ...
    def l_select(self):
        ...
        if self.dao.l_id_from_member(mem.MemService.login_user_id)!=None:
            print('이미 락커를 사용중입니다')
            return
        elif (self.dao.l_id_from_member(mem.MemService.login_user_id)==None) and (l_id > 20 or l_id<0):
            ...
            if self.dao.selectByL_id(l_id).l_status=='이용가능':
                ...
                self.memdao.l_id_update(l_id,mem.MemService.login_user_id)
            else:
                print('이미 이용중인 자리입니다')
                return
    ...
    def edit_LockerInfo(self):
        print('락커 비밀번호 수정')
        l_id = int(input('락커 번호 선택: '))
        if self.dao.selectByL_id(l_id).l_status=='이용중' and (self.dao.l_id_from_member(mem.MemService.login_user_id)==l_id):
            new_l_pw = input('새로운 비밀번호: ')
            self.dao.update(l_id, new_l_pw)
        else:
            print('비어있는 락커 혹은 본인의 락커가 아닙니다')
            return
    ...
    def del_LockerInfo(self):
        ...
        if vo == None:
            print('비어있는 락커번호 입니다')
        elif vo.l_status == '이용중' and (self.dao.l_id_from_member(mem.MemService.login_user_id)==l_id) :
            l_pw = input('비밀번호 입력: ')
            if vo.l_pw == l_pw:
                self.dao.update2(l_id)
                self.memdao.l_id_update(None, mem.MemService.login_user_id)
                print('락커 해지 완료')
            else:
