...
class Memdao:
    ...
    def mem_select(self, id):
        ...
        self.conn.commit()
        if row != None:
            vo = MemVo(row[0], row[1], row[2], row[3], row[4], row[5])
            return vo
    ...
    def select_user_prg1(self):  # 본인이 신청한 프로그램 조회. 회원 조회시, 수강신청, 취소시 사용
        members = []
        self.connect()
        cur = self.conn.cursor()
    ...
    def l_id_update(self,l_id,mem_user_id):
        self.connect()
        cur = self.conn.cursor()
        sql = "update member set l_id = %s  where mem_user_id = %s"
        vals = (l_id, mem_user_id )
        cur.execute(sql, vals)
        self.conn.commit()
...
class MemService:
    ...
    def mem_editMemInfo(self):
        ...
        if m == '1':
            new_pwd = input('새 pwd:')
            self.dao.mem_editpwd(MemService.login_user_id, new_pwd)
